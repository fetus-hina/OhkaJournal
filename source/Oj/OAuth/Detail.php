...
class Oj_OAuth_Detail {
    ...
    static private function parameterSources(Zend_Uri_Http $uri, array $oauth_params, $http_content) {
        ...
        // リクエストボディのコピー
        if($http_content != '') {
            if(is_array($http_content)) {
                foreach($http_content as $key => $val) {
                    if(!isset($parameters[$key])) {
                        $parameters[$key] = array();
                    }
                    $parameters[$key][] = $val;
    ...
    static private function parameterNormalization(array $parameters) {
        $result = array();
        uksort($parameters, function($a,$b){return strcmp(Oj_OAuth_Detail::percentEncoding($a), Oj_OAuth_Detail::percentEncoding($b));});
        foreach($parameters as $key => $value) {
            if(is_array($value)) {
                usort($value, function($a, $b){return strcmp(Oj_OAuth_Detail::percentEncoding($a), Oj_OAuth_Detail::percentEncoding($b));});
                foreach($value as $v) {
                    $result[] = sprintf('%s=%s', self::percentEncoding($key), self::percentEncoding($v));
                }
            } else {
                $result[] = sprintf('%s=%s', self::percentEncoding($key), self::percentEncoding($value));
            }
        }
